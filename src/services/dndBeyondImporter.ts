...
import { hashData, compareHashes } from '../utils/crypto';
import type { DndBeyondCharacter, NormalizedCharacter } from '../types/dndBeyond';
import { CharacterSyncStatus, AuditAction } from '@prisma/client';
...
export class DndBeyondImporter {
   ...
   */
  async fetchCharacterFromApi(characterId: string, characterUrl?: string): Promise<DndBeyondCharacter> {
    // Try to fetch from public JSON endpoint if URL provided
    if (characterUrl) {
      try {
        const jsonUrl = characterUrl.replace(/\/$/, '') + '/json';
        const response = await axios.get<DndBeyondCharacter>(jsonUrl, {
          headers: {
        ...
        if (axios.isAxiosError(error)) {
          if (error.response?.status === 403 || error.response?.status === 401) {
            throw new Error(
              'Unable to access character sheet. Please ensure the character sheet is set to PUBLIC in D&D Beyond settings.'
            );
          }
    ...
    if (this.apiClient) {
      try {
        const response = await this.apiClient.get<DndBeyondCharacter>(`/characters/${characterId}`);
        return response.data;
      } catch (error) {
        if (axios.isAxiosError(error)) {
    ...
    throw new Error(
      'Character URL is required. Please provide the full D&D Beyond character sheet URL.'
    );
  }
  ...
  normalizeCharacter(rawData: any): NormalizedCharacter {
    ...
    if (data.stats) {
      data.stats.forEach((stat: any) => {
        ...
        if (id) statsMap[id] = value;
      });
    }
    // Alternative format: direct stat values
    if (data.strength !== undefined) statsMap[1] = data.strength;
    if (data.dexterity !== undefined) statsMap[2] = data.dexterity;
    if (data.constitution !== undefined) statsMap[3] = data.constitution;
    if (data.intelligence !== undefined) statsMap[4] = data.intelligence;
    if (data.wisdom !== undefined) statsMap[5] = data.wisdom;
    if (data.charisma !== undefined) statsMap[6] = data.charisma;

    // Apply modifiers if present
    if (data.modifiers?.race) {
      data.modifiers.race.forEach((mod: any) => {
    ...
    if (data.modifiers?.class) {
      data.modifiers.class.forEach((mod: any) => {
        if (statsMap[mod.statId] !== undefined) {
          statsMap[mod.statId] += mod.value;
        }
      });
    ...
    if (data.classes && Array.isArray(data.classes) && data.classes.length > 0) {
      ...
      level = data.classes.reduce((sum: number, c: any) => sum + (c.level || 0), 0);
    ...
    const equipment = itemsArray.map((item: any) => {
    ...
    let spells: any[] | undefined = undefined;
    if (data.spells) {
      if (Array.isArray(data.spells)) {
        ...
        const spellsArray: any[] = [];
        Object.values(data.spells).forEach((spellGroup: any) => {
          if (Array.isArray(spellGroup)) {
            spellsArray.push(...spellGroup);
          }
        });
        spells = spellsArray.map((spell: any) => ({
          name: spell.name || spell.spellName || 'Unknown Spell',
          level: spell.level || spell.spellLevel || 0,
          school: spell.school || spell.schoolName,
        }));
  ...
  async importCharacter(
    ...
    rawData?: DndBeyondCharacter,
    characterUrl?: string
  ): Promise<{ characterId: string; isNew: boolean; isUpdated: boolean }> {
    ...
    if (!characterData) {
      try {
        ...
        throw new Error(
          'Unable to fetch character data from D&D Beyond. ' +
    ...
    console.log('[DndBeyondImporter] characterData before normalization:', {
      ...
      hasName: !!(characterData as any).name,
    ...
    const rawHash = hashData(characterData);
    ...
    if (existing) {
      ...
      const hasChanged = !existing.lastDndBeyondHash || 
        !compareHashes(existing.lastDndBeyondHash, rawHash);
      ...
      if (hasChanged) {
        ...
        await prisma.character.update({
          ...
          data: {
            ...
            syncHash: hashData(normalized),
            lastDndBeyondHash: rawHash,
            lastSyncAt: new Date(),
            syncStatus: CharacterSyncStatus.SYNCED,
          },
        });
        isUpdated = true;
      } else {
      ...
      await prisma.auditLog.create({
        data: {
          action: AuditAction.CHARACTER_IMPORT,
          userId,
   ...
   */
  async checkSyncStatus(characterId: string): Promise<{
    needsSync: boolean;
    ...
    try {
      ...
      const newHash = hashData(rawData);
      
      const needsSync = !character.lastDndBeyondHash || 
        !compareHashes(character.lastDndBeyondHash, newHash);
...
export const dndBeyondImporter = new DndBeyondImporter();
