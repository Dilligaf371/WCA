...
export interface DndBeyondCharacter {
  ...
  classes?: Array<{
    ...
    subclass?: { name: string; id?: number };
    level: number;
  }>;
  level: number;
  background?: {
  ...
  modifiers?: {
    race?: Array<{ statId: number; value: number }>;
    class?: Array<{ statId: number; value: number }>;
  };
  hitPoints?: {
    max?: number;
    current?: number;
  ...
  proficiencies?: {
    skills?: Array<{
      skillId: number;
      proficiencyType?: number; // 0=none, 1=proficient, 2=expertise
    }>;
  };
  equipment?: Array<{
    id?: number;
